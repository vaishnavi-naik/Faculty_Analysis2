...
if(mysqli_num_rows($res) == 1){
	...
	if($fetchedPass == $oldPass){
		...
		if(mysqli_query($connect, $query2)  or die(mysqli_error($connect))){
			header('location:dashboard_new.php?msg="Password Changed Sucessfully!"#changePass');
		}else header('location:dashboard_new.php?insertError="Data not Updated"#changePass');
	}else header('location:dashboard_new.php?error="Incorrect Current Password entered!"#changePass');
}else header('location:dashboard_new.php?passError="Duplicate Entries Found!"#changePass');
?>